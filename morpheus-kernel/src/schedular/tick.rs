...
static mut AP_IDLE_CTX: [CpuContext; MAX_CPUS] = [const { CpuContext::zeroed() }; MAX_CPUS];
...
unsafe fn ap_quiesce_hlt_loop(core_idx: u32) -> ! {
    hal().smp().shutdown_quiesce_ack(core_idx);
...
pub unsafe extern "C" fn scheduler_tick(current_ctx: &CpuContext) -> &'static CpuContext {
    ...
    let now_tsc = hal().timer().read_tsc();

    if core_idx == 0 {
        sched_hooks::fb_present_tick();
    ...
    if cur_pid == 0 && core_idx != 0 {
        ...
        if next_pid == 0 {
            ...
            let boot_rsp = hal().smp().pcpu_boot_kernel_rsp();
            hal().smp().set_kernel_stack_for_core(core_idx, boot_rsp);
            hal().smp().pcpu_set_kernel_syscall_rsp(boot_rsp);
    ...
    if let Some(Some(cur)) = PROCESS_TABLE.get_mut(cur_pid) {
        ...
        // through the HAL ctx_is_user_mode / ctx_set_user_mode predicate pair.
        let h = hal();
        if h.cpu().ctx_is_user_mode(&cur.context) {
            h.cpu().ctx_set_user_mode(&mut cur.context, true);
        }
    ...
    let compositor_active = sched_hooks::compositor_active();
...
pub(super) unsafe fn deliver_pending_signals(pid: u32) {
    ...
    while let Some(sig) = proc.pending_signals.take_next() {
        ...
        if handler > 1 {
            ...
            // CpuContext is opaque (morpheus-hal-api); route through HAL
            // accessors for rip/rsp + arg0 (rdi on x86_64).
            let h = hal();
