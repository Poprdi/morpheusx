//! Arch-portable kernel spinlocks. IF management and per-core indexing route through
//! the HAL `Cpu`/`Smp` traits.
...
use crate::hal;
use morpheus_foundation::MAX_CPUS;
...
const MAX_CPUS_USIZE: usize = MAX_CPUS as usize;

/// IF save/restore in per-core slot keyed off `Smp::current_core_index`. Cap = `MAX_CPUS`.
pub struct IsrSafeRawSpinLock {
    locked: AtomicBool,
    /// Only the holding core touches its slot.
    saved_if: [UnsafeCell<bool>; MAX_CPUS_USIZE],
}
...
impl IsrSafeRawSpinLock {
    pub const fn new() -> Self {
        // Interior-mutable const used solely as an array initializer below.
        #[allow(clippy::declare_interior_mutable_const)]
        const CELL_FALSE: UnsafeCell<bool> = UnsafeCell::new(false);
        Self {
            locked: AtomicBool::new(false),
            saved_if: [CELL_FALSE; MAX_CPUS_USIZE],
        }
    }

    #[inline(always)]
    fn core_index() -> usize {
        (hal().smp().current_core_index() as usize) & (MAX_CPUS_USIZE - 1)
    }
