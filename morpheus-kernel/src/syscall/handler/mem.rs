...
unsafe fn mmap_locked(pages: u64) -> u64 {
    ...
    // Zero before exposing to user (no kernel data leak).
...
pub unsafe fn sys_munmap(vaddr: u64, pages: u64) -> u64 {
    ...
    // Same per-address-space lock as sys_mmap.
    let lock = SCHEDULER.current_address_space_lock();
    lock.lock();
...
unsafe fn munmap_locked(vaddr: u64, pages: u64) -> u64 {
    ...
    proc.vma_table.remove(idx); // remove before unmap to prevent double-free on partial failure
    ...
    let _ = PAGE_SIZE; // suppress unused-import warning
