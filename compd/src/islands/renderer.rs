...
pub fn compose(state: &mut CompState) {
    ...
    if let Some(di) = state.desktop_idx {
        if let Some(ref dw) = state.windows[di] {
            if dw.mapped && !dw.surface_ptr.is_null() {
                blit_surface(state, fb_ptr, 0, 0, state.fb_w, state.fb_h,
                             dw.src_w, dw.src_h, dw.src_stride, dw.surface_ptr,
                             dw.surface_pages);
                drew_desktop = true;
            }
        }
    ...
    if !drew_desktop {
        ...
        raw_fill(fb_ptr, state.fb_stride, 0, 0, state.fb_w, state.fb_h,
                 state.pack(dr, dg, db));
    }
    ...
    if let Some(di) = state.desktop_idx {
        if let Some(ref dw) = state.windows[di] {
            if dw.mapped && !dw.surface_ptr.is_null() {
                ...
                blit_strip(state, fb_ptr, dw.surface_ptr, dw.src_stride, dw.surface_pages,
                           0, panel_y, state.fb_w, PANEL_H);
            }
        }
    }
...
fn draw_window(state: &mut CompState, idx: usize, focused: bool) {
    ...
    let (win_x, win_y, win_w, win_h, win_src_w, win_src_h, win_src_stride,
         win_mapped, win_surface_ptr, title_buf, title_len) = {
        let win = state.windows[idx].as_ref().unwrap();
        (win.x, win.y, win.w, win.h, win.src_w, win.src_h, win.src_stride,
         win.mapped, win.surface_ptr, win.title, win.title_len)
    };

    let (tb_r, tb_g, tb_b) = if focused { TITLE_FOCUSED_RGB } else { TITLE_UNFOCUSED_RGB };
    let (br, bg, bb) = if focused { BORDER_FOCUSED_RGB } else { BORDER_UNFOCUSED_RGB };
    ...
    // border: left
    clip_fill(state, fb_ptr, outer_x, outer_y + BORDER as i32,
              BORDER, outer_h - BORDER, br, bg, bb);
    // border: right
    clip_fill(state, fb_ptr,
              outer_x + outer_w as i32 - BORDER as i32,
              outer_y + BORDER as i32,
              BORDER, outer_h - BORDER, br, bg, bb);
    // border: bottom
    clip_fill(state, fb_ptr, outer_x, outer_y + outer_h as i32 - BORDER as i32,
              outer_w, BORDER, br, bg, bb);
    ...
    draw_text(state, fb_ptr, text_x as u32, text_y as u32, title_str,
              TITLE_TEXT_RGB, (tb_r, tb_g, tb_b));

    // close button "[X]"
    let close_x = (tb_x + tb_w as i32 - 32).max(0);
    draw_text(state, fb_ptr, close_x as u32, text_y as u32, "[X]",
              TITLE_TEXT_RGB, (tb_r, tb_g, tb_b));
    ...
    if win_mapped && !win_surface_ptr.is_null() {
        blit_surface(state, fb_ptr, win_x, win_y, win_w, win_h,
                     win_src_w, win_src_h, win_src_stride, win_surface_ptr,
...
fn blit_surface(
    state: &CompState, fb_ptr: *mut u32,
    dst_x: i32, dst_y: i32, dst_w: u32, dst_h: u32,
    _src_w: u32, _src_h: u32, src_stride: u32,
    surface_ptr: *const u32, surface_pages: u64,
) {
    ...
    unsafe {
        for dy in y0..y1 {
            ...
            if (src_row + src_x_off) as usize >= mapped_pixels { break; }

            for dx in x0..x1 {
                let sx = src_x_off + (dx - x0);
                let src_off = (src_row + sx) as usize;
                if src_off >= mapped_pixels { break; }
...
fn blit_strip(
    state: &CompState, fb_ptr: *mut u32,
    surface_ptr: *const u32, src_stride: u32, surface_pages: u64,
...
fn clip_fill(state: &CompState, fb_ptr: *mut u32,
...
fn draw_text(
    ...
    for (ci, ch) in text.chars().enumerate() {
        ...
        raw_glyph(fb_ptr, state.fb_stride, gx, y, glyph, fg_px, bg_px, state.fb_h);
    }
}
...
fn draw_cursor(state: &CompState, fb_ptr: *mut u32) {
    ...
    for d in -4i32..=4 {
        raw_put(fb_ptr, state.fb_stride, state.fb_w, state.fb_h, cx + d, cy, px);
        raw_put(fb_ptr, state.fb_stride, state.fb_w, state.fb_h, cx, cy + d, px);
    }

    let outline = state.pack(0, 0, 0);
    for d in [-5i32, 5] {
        raw_put(fb_ptr, state.fb_stride, state.fb_w, state.fb_h, cx + d, cy, outline);
        raw_put(fb_ptr, state.fb_stride, state.fb_w, state.fb_h, cx, cy + d, outline);
    }
}
