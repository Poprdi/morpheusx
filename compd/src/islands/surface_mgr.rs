...
pub fn update(state: &mut CompState) {
    ...
    // if this returns u64::MAX, compd isn't up yet. but we ARE compd. so this shouldn't happen.
    if count == usize::MAX { return; }

    // build a bitset of PIDs present in the current surface list
    ...
    // reap windows whose PID is no longer in surface_list.
    // compd isn't the parent, so try_wait returns ESRCH. instead we just check presence.
            ...
            if let Some(ref mut win) = state.windows[idx] {
                if !win.mapped {
                    ...
                    if let Ok(ptr) = compsys::surface_map(entry.pid) {
                        ...
                        // stride in surface entries is pixels. not bytes. yes again. yes i know.
