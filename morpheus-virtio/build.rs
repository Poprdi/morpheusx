//! Assembles `asm/virtio/*.s` into `libmorpheus_virtio_asm.a`.
...
fn main() {
    ...
    println!(
        "cargo:warning=Building morpheus-virtio ASM for: {} ({})",
        target, obj_format
    );

    let mut objects = Vec::new();

    for path in ASM_VIRTIO.iter() {
    ...
    let lib = out_dir.join("libmorpheus_virtio_asm.a");
    ...
    println!("cargo:rustc-link-lib=static=morpheus_virtio_asm");
}
