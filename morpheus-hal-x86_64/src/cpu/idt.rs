...
/// Process lookup hook for crash diagnostics. Returns (pid, name_bytes).
/// `name` is filled with up to 32 NUL-padded bytes; `name_len` is the actual
/// usable byte count (NUL strip allowed). Kernel installs at boot.
pub type ProcessLookupFn = unsafe fn(pid: u32, name: &mut [u8; 32]) -> bool;

/// Current PID accessor. Returns 0 if no scheduler is up yet.
pub type CurrentPidFn = unsafe fn() -> u32;

/// User-mode process exit on fault. Never returns.
pub type ProcessExitFn = unsafe fn(code: i32) -> !;

static mut PROCESS_LOOKUP_HOOK: Option<ProcessLookupFn> = None;
static mut CURRENT_PID_HOOK: Option<CurrentPidFn> = None;
static mut PROCESS_EXIT_HOOK: Option<ProcessExitFn> = None;
...
/// Install the process-lookup hook used by the crash dumper.
///
/// # Safety
/// Single-threaded init only.
pub unsafe fn set_process_lookup_hook(hook: ProcessLookupFn) {
    PROCESS_LOOKUP_HOOK = Some(hook);
}

/// Install the current-pid hook used by the crash dumper.
///
/// # Safety
/// Single-threaded init only.
pub unsafe fn set_current_pid_hook(hook: CurrentPidFn) {
    CURRENT_PID_HOOK = Some(hook);
}

/// Install the process-exit hook used to terminate user faults.
///
/// # Safety
/// Single-threaded init only.
pub unsafe fn set_process_exit_hook(hook: ProcessExitFn) {
    PROCESS_EXIT_HOOK = Some(hook);
}
...
pub extern "C" fn exception_handler(
    ...
    let found = unsafe {
        match PROCESS_LOOKUP_HOOK {
            Some(h) => h(pid, &mut proc_name),
            None => false,
        }
    };
    if !found {
    ...
    if is_user_mode {
        ...
        unsafe {
            ...
            // No exit hook installed (very early boot): fall through to crash
            // hook + halt below.
        }
    }
...
pub use crate::intr::{disable_interrupts, enable_interrupts, interrupts_enabled};
