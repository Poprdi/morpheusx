...
#[inline(always)]
unsafe fn io_wait() {
    crate::asm::pio::outb(0x80, 0);
}

#[inline(always)]
unsafe fn kbc_wait_input_empty() {
    for _ in 0..100_000 {
        if (crate::asm::pio::inb(0x64) & 0x02) == 0 {
            return;
        }
        core::hint::spin_loop();
    }
...
unsafe fn reset_via_cf9() {
    crate::asm::pio::outb(0xCF9, 0x02);
    io_wait();
    crate::asm::pio::outb(0xCF9, 0x06);
}

#[inline(always)]
unsafe fn reset_via_port92() {
    let mut v = crate::asm::pio::inb(0x92);
    v &= !0x01;
    crate::asm::pio::outb(0x92, v);
    io_wait();
    crate::asm::pio::outb(0x92, v | 0x01);
}

#[inline(always)]
unsafe fn reset_via_8042() {
    kbc_wait_input_empty();
    crate::asm::pio::outb(0x64, 0xFE);
}
...
pub unsafe fn wait_for_keypress_or_timeout_ms(timeout_ms: u64) {
    ...
    loop {
        let status = crate::asm::pio::inb(0x64);
        if (status & 0x01) != 0 {
            let _ = crate::asm::pio::inb(0x60);
            break;
        }
