//! Assembles `asm/cpu/*.s` into `libmorpheus_x86_asm.a`.
...
fn main() {
    ...
    println!(
        "cargo:warning=Building morpheus-x86-asm ASM for: {} ({})",
        target, obj_format
    );

    let mut objects = Vec::new();

    for path in ASM_CPU.iter() {
    ...
    let lib = out_dir.join("libmorpheus_x86_asm.a");
    ...
    println!("cargo:rustc-link-lib=static=morpheus_x86_asm");
}
