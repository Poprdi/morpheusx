...
fn rename_to_overlong_path_is_rejected() {
    ...
    assert!(fs.rename(&mut dev, "/a", &long, 2).is_err(), "overlong rename must fail");
    assert!(fs.stat("/a").is_ok(), "source must survive a rejected rename");
}
...
fn rename_onto_existing_file_frees_destination() {
    ...
    assert_eq!(fs.read(&mut dev, "/b").unwrap(), vec![0xAAu8; 2 * BLOCK], "rename target wrong");
    assert!(fs.read(&mut dev, "/a").is_err(), "source must be gone after rename");
    assert_eq!(
...
fn rename_directory_moves_children() {
    ...
    assert_eq!(fs.read(&mut dev, "/e/f").unwrap(), b"hello", "child not moved");
    assert_eq!(fs.read(&mut dev, "/e/sub/g").unwrap(), b"world", "nested child not moved");
    assert!(fs.stat("/d").is_err(), "old directory must be gone");
    assert!(fs.read(&mut dev, "/d/f").is_err(), "old child path must be gone");

    fs.sync(&mut dev).unwrap();
    drop(fs);
    let fs2 = HelixFs::mount(&mut dev, 0, 512).unwrap();
    assert_eq!(fs2.read(&mut dev, "/e/sub/g").unwrap(), b"world", "subtree rename lost on remount");
}
