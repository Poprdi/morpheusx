...
fn log_full_self_heals_via_checkpoint() {
    ...
    for i in 0..N {
        ...
        assert!(
            r.is_ok(),
            "write {i} bricked the log (LogFull not self-healed): {r:?}"
        );
    }

    let last = (N - 1).to_le_bytes().to_vec();
    assert_eq!(
        fs.read(&mut dev, "/a").unwrap(),
        last,
        "live content wrong after checkpointing"
    );
...
fn dir_ops_self_heal_on_log_full() {
    ...
    for i in 0..15_000u32 {
        fs.mkdir(&mut dev, "/d", 1)
            .unwrap_or_else(|e| panic!("mkdir iter {i}: {e:?}"));
        fs.unlink(&mut dev, "/d", 1)
            .unwrap_or_else(|e| panic!("unlink iter {i}: {e:?}"));
    }
    ...
    assert!(
        fs2.stat("/keep").is_ok(),
        "dir lost across checkpoint + remount"
    );
}
