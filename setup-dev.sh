...
# Keyboard layouts: generated by keymap-gen and injected into the HelixFS so
# the compositor can load (and hot-swap) them. (src-in-KEYMAP_DIR, dest).
# de.kmap is also written to /system/keymap.kmap — the active layout compd reads.
KEYMAP_DIR="target/keymaps"
KEYMAP_FILES=(
    "de.kmap,/system/keymap.kmap"
    "de.kmap,/system/keymaps/de.kmap"
    "us.kmap,/system/keymaps/us.kmap"
)

# Build the host keymap generator and emit the .kmap files into KEYMAP_DIR.
do_build_keymaps() {
    log_info "Building keymap-gen (host) + generating layouts..."
    cargo build --release -p keymap-gen 2>&1 || { log_error "keymap-gen build failed"; return 1; }
    "${PROJECT_ROOT}/target/release/keymap-gen" "${KEYMAP_DIR}" 2>&1 \
        || { log_error "keymap generation failed"; return 1; }
}
...
do_inject_apps() {
    ...
    for entry in "${KEYMAP_FILES[@]}"; do
        local src="${KEYMAP_DIR}/${entry%%,*}"
        local kdest="${entry##*,}"
        if [[ ! -f "$src" ]]; then
            log_warn "keymap ${src} missing, skipping inject"
            continue
        fi
        log_info "Injecting keymap ${src} → ${kdest} (HelixFS partition)..."
        sudo "${PROJECT_ROOT}/target/release/morpheus-cli" inject "$data_part" "$src" --dest "$kdest" 2>&1 \
            || {
                sudo losetup -d "$loopdev" 2>/dev/null || true
                log_error "Inject failed for keymap ${src}"
                return 1
            }
        injected=$((injected + 1))
    done
...
cmd_flash() {
    ...
    do_build_keymaps || die "keymap generation failed"
    ...
    for entry in "${KEYMAP_FILES[@]}"; do
        ...
        local kdest="${entry##*,}"
        if [[ ! -f "$src" ]]; then
            log_warn "keymap ${src} missing, skipping"
            continue
        fi
        log_info "Injecting keymap ${src} → ${kdest}"
        sudo "${PROJECT_ROOT}/target/release/morpheus-cli" inject "$helix_part" "$src" --dest "$kdest" 2>&1 \
            || { log_error "Inject failed for keymap ${src}"; continue; }
        injected=$((injected + 1))
    done
