//! Futex-based sync primitives: Mutex, Condvar, OnceLock, RwLock, mpsc.
//! Built on SYS_FUTEX so blocking is kernel-scheduled, not spun.
...
/// Blocks if `*addr == expected`. Returns when woken or value changed.
#[inline]
...
/// Wakes up to `count` waiters. Returns number woken.
#[inline]
...
/// 3-state protocol avoids thundering-herd wakes.
pub struct Mutex<T> {
    state: AtomicU32,
    data: UnsafeCell<T>,
}
...
impl<T> Mutex<T> {
    ...
    pub fn lock(&self) -> MutexGuard<'_, T> {
        ...
        // Slow path: swap to 2 (contended) and sleep.
        loop {
            let old = self.state.swap(2, Ordering::Acquire);
            if old == 0 {
                // Won the race between CAS failure and the swap.
                return MutexGuard { mutex: self };
            }
    ...
    fn unlock_inner(&self) {
        // Wake only if waiters existed (state was 2).
...
/// Pair with a Mutex for wait/notify.
pub struct Condvar {
    seq: AtomicU32,
}
...
impl Condvar {
    ...
    /// Releases mutex, sleeps until notified, re-acquires.
    pub fn wait<'a, T>(&self, guard: MutexGuard<'a, T>) -> MutexGuard<'a, T> {
        let seq = self.seq.load(Ordering::Relaxed);
        let mutex = guard.mutex;
        // Must drop guard BEFORE futex_wait.
        core::mem::drop(guard);
...
impl<T> OnceLock<T> {
    ...
    pub fn get_or_init(&self, f: impl FnOnce() -> T) -> &T {
            ...
            .is_ok()
        {
            ...
            // Initializer race lost; wait for state=2.
...
/// State: `0`=unlocked, `1..=0x7FFF_FFFE`=N readers, `0xFFFF_FFFF`=writer.
pub struct RwLock<T> {
    state: AtomicU32,
    data: UnsafeCell<T>,
}
...
/// Unbounded MPSC built on `Mutex<VecDeque<T>>` + `Condvar`.
pub mod mpsc {
    use super::*;
    ...
    struct Inner<T> {
        ...
        not_empty: Condvar,
        /// Live sender count. 0 means receiver returns None.
        senders: AtomicU32,
    }
    ...
    impl<T> Receiver<T> {
        /// Returns `None` when all senders dropped and queue empty.
