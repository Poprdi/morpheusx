...
impl XhciController {
    ...
    pub unsafe fn enable_slot(&mut self) -> Result<u8, XhciError> {
        ...
        vw64(self.dma_base + dma::OFF_DCBAA as u64 + (slot as u64) * 8, out_ctx);
        Ok(slot)
    }
    ...
    pub unsafe fn address_device(
        ...
        vw32(slot_ctx + 4, ((port as u32 + 1) << 16) & !(0xFF << 16) | ((port as u32 + 1) << 16));
    ...
    /// Fetch device descriptor (18 bytes).
    pub unsafe fn get_device_descriptor(
        ...
        crate::usb::control::ControlXfer {
            ep0: ctrl,
            slot_id: self.slot_id,
        }
        .control_in(0x80, 0x06, 0x0100, 0, 18, desc_buf, &mut |sid, dci| {
            self.ring_xfer_doorbell(dci);
    ...
    pub unsafe fn get_config_descriptor(
        &mut self,
        len: u16,
    ) -> Result<*const u8, XhciError> {
        let desc_buf = self.dma_base + dma::OFF_DESC as u64;
        ...
        .control_in(0x80, 0x06, 0x0200, 0, len, desc_buf, &mut |sid, dci| {
            self.ring_xfer_doorbell(dci);
            self.wait_xfer(sid, dci, 5000)
        })?;
        Ok(desc_buf as *const u8)
    ...
    pub unsafe fn set_configuration(
        ...
        .control_nodata(0x00, 0x09, cfg_val as u16, 0, &mut |sid, dci| {
            self.ring_xfer_doorbell(dci);
            self.wait_xfer(sid, dci, 5000)
        })?;
        Ok(())
    ...
    pub unsafe fn parse_config(
        &self,
        desc_ptr: *const u8,
    ) -> Option<(u8, u8, u8, u16, u16)> {
        let d = desc_ptr as u64;
    ...
    pub unsafe fn reset_transfer_state(&mut self) {
        core::ptr::write_bytes((self.dma_base + dma::OFF_XFER_EP0 as u64) as *mut u8, 0, 256);
        core::ptr::write_bytes((self.dma_base + dma::OFF_XFER_BOUT as u64) as *mut u8, 0, 256);
        core::ptr::write_bytes((self.dma_base + dma::OFF_XFER_BIN as u64) as *mut u8, 0, 256);
        core::ptr::write_bytes((self.dma_base + dma::OFF_OUT_CTX as u64) as *mut u8, 0, 2048);
