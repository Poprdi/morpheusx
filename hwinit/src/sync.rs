//! Spinlocks, once-init, and an RAII interrupt guard.
...
/// No interrupt management — caller deals with IF state.
...
/// IsrSafe variant: saves/restores IF via per-core slot keyed off gs:[0x00].
/// Caps at 64 cores. Before per-CPU is up, gs base is 0 and BSP gets slot 0.
pub struct IsrSafeRawSpinLock {
    locked: AtomicBool,
    /// Only the holding core touches its own slot.
...
/// Disables interrupts on construct, restores prior IF on drop.
