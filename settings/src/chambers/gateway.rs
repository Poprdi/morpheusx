...
pub fn render(app: &SettingsApp) {
    ...
    // welcome header
    widgets::draw_str(s, st, px, cy, "Oneiric Gateway", t.signal, t.substrate, w, h);
    cy += widgets::FONT_H + 4;
    ...
    // chamber links
    layout::draw_section(app, px, cy, "Chambers");
    cy += widgets::FONT_H + 4;
    ...
    for (i, route) in Route::ALL.iter().enumerate() {
        ...
        widgets::fill_rect(s, st, px, cy, row_w, widgets::FONT_H + 4, bg, w, h);
        if is_focused {
            widgets::rect_outline(s, st, px, cy, row_w, widgets::FONT_H + 4, t.focus_ring, w, h);
        }
        ...
        widgets::draw_str(s, st, px + 4, cy + 2, num_str, t.glyph_dim, bg, w, h);

        // sigil
        widgets::draw_str(s, st, px + 3 * widgets::FONT_W, cy + 2, sigil, t.signal, bg, w, h);

        // label
        widgets::draw_str(s, st, px + 5 * widgets::FONT_W, cy + 2, label, fg, bg, w, h);

        // alias
        let alias = route.technical_alias();
        let alias_x = px + 25 * widgets::FONT_W;
        widgets::draw_str_trunc(s, st, alias_x, cy + 2, alias, t.glyph_dim, bg, w, h, 30);
