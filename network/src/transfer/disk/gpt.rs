...
use super::types::{DiskError, DiskResult, PartitionInfo, guid, SECTOR_SIZE};
...
impl GptOps {
    ...
    pub fn scan_partitions<B: BlockIo>(
        ...
        block_io.read_blocks(Lba(1), &mut header_buf)
        ...
        for i in 0..32 {
            ...
            block_io.read_blocks(Lba(entry_lba + i as u64), sector_buf)
                .map_err(|_| DiskError::IoError)?;
        }
    ...
    pub fn create_partition<B: BlockIo>(
        ...
        for i in 0..32 {
            ...
            block_io.write_blocks(Lba(entry_lba + i as u64), sector_buf)
                .map_err(|_| DiskError::IoError)?;
        }
        
        // Write back header
        block_io.write_blocks(Lba(1), &header_buf)
            .map_err(|_| DiskError::IoError)?;
