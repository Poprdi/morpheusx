...
impl<'a> VirtioBlkBlockIo<'a> {
    ...
    fn sync_read(&mut self, sector: u64, num_sectors: u32, dst: &mut [u8]) -> Result<(), BlockIoError> {
        ...
        self.driver.submit_read(sector, self.dma_buffer_phys, num_sectors, request_id)
    ...
    fn sync_write(&mut self, sector: u64, num_sectors: u32, src: &[u8]) -> Result<(), BlockIoError> {
        ...
        self.driver.submit_write(sector, self.dma_buffer_phys, num_sectors, request_id)
            .map_err(BlockIoError::DriverError)?;
...
impl<'a> BlockIo for VirtioBlkBlockIo<'a> {
    ...
    fn read_blocks(&mut self, start_lba: Lba, dst: &mut [u8]) -> Result<(), Self::Error> {
        ...
        while remaining > 0 {
            ...
            self.sync_read(current_sector, chunk_sectors, &mut dst[offset..offset + chunk_bytes])?;
    ...
    fn write_blocks(&mut self, start_lba: Lba, src: &[u8]) -> Result<(), Self::Error> {
        ...
        while remaining > 0 {
            ...
            self.sync_write(current_sector, chunk_sectors, &src[offset..offset + chunk_bytes])?;
