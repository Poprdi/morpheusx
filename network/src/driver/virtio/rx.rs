...
use crate::types::{VirtqueueState, RxResult, VirtioNetHdr};
...
pub fn receive(
    ...
    if payload_len > out_buffer.len() {
        ...
        resubmit_buffer(rx_state, rx_pool, result.buffer_idx);
        return Err(RxError::BufferTooSmall { needed: payload_len });
    }
    
    // Copy frame (skip VirtIO header)
    out_buffer[..payload_len].copy_from_slice(
        &buf.as_slice()[VirtioNetHdr::SIZE..VirtioNetHdr::SIZE + payload_len]
    );
...
fn resubmit_buffer(
    ...
    use crate::asm::drivers::virtio::{rx as asm_rx, notify};
