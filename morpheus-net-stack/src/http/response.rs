...
impl Response {
    ...
    /// Returns `(response, bytes consumed)`. Body is truncated by available
    /// input if shorter than Content-Length.
    pub fn parse(data: &[u8]) -> Result<(Self, usize)> {
        // HTTP/1.1 is ASCII; UTF-8 validation suffices.
    ...
    /// HEAD responses or stream-mode parsing. Returns body offset, not body.
