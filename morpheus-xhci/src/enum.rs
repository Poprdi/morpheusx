...
use crate::dma;
use crate::regs::*;
use crate::rings::{vr32, vw32, vw64};
    ...
    pub unsafe fn enable_slot(&mut self) -> Result<u8, XhciError> {
        ...
        let (slot, _) = match self.wait_cmd(2000) {
            ...
            Err(XhciError::CommandTimeout) => {
                ...
                crate::logger::warn("USB", 251, "enable_slot: no completion event (timeout)");
            ...
            Err(e) => {
                ...
                crate::logger::warn("USB", 252, "enable_slot: command returned error CC");
                return Err(e);
            },
        };
        if slot == 0 {
            ...
            crate::logger::warn("USB", 253, "enable_slot: success event but slot id = 0");
            return Err(XhciError::EnableSlotFailed);
        }
        if (slot as usize) > dma::MAX_OUT_CTX_SLOTS {
            crate::logger::warn("USB", 254, "enable_slot: slot id exceeds out-ctx capacity");
            return Err(XhciError::EnableSlotFailed);
        }
        self.slot_id = slot;
