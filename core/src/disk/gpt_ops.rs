...
use gpt_disk_types::{BlockSize, GptHeader, GptPartitionEntry, GptPartitionEntryArray, LbaLe, U32Le, guid};
use gpt_disk_io::{BlockIo, Disk};
use crate::disk::partition::{PartitionInfo, PartitionType, PartitionTable};
...
pub fn create_gpt<B: BlockIo>(block_io: B, num_blocks: u64) -> Result<(), GptError> {
    ...
    let mut buf = [0u8; 512];
    disk.write_protective_mbr(&mut buf).map_err(|_| GptError::IoError)?;
    
    // Write primary GPT header
    disk.write_primary_gpt_header(&header, &mut buf).map_err(|_| GptError::IoError)?;
    
    // Write empty partition array
    let layout = header.get_partition_entry_array_layout()
    ...
    disk.write_gpt_partition_entry_array(&entry_array).map_err(|_| GptError::IoError)?;
...
pub fn find_free_space<B: BlockIo>(
    ...
    let header = disk.read_primary_gpt_header(&mut [0u8; 512])
        .map_err(|_| GptError::InvalidHeader)?;
    ...
    let iter = disk.gpt_partition_entry_array_iter(layout, entry_buffer)
        .map_err(|_| GptError::IoError)?;
    ...
    for entry_result in iter {
        ...
        if used_count < 16 {
            used_ranges[used_count] = (
                entry.starting_lba.to_u64(),
                entry.ending_lba.to_u64(),
            );
            used_count += 1;
        }
    }
...
pub fn create_partition<B: BlockIo>(
    ...
    let mut header = disk.read_primary_gpt_header(&mut [0u8; 512])
        .map_err(|_| GptError::InvalidHeader)?;
    ...
    let mut entry_buf = [0u8; 16384];
    let mut entry_array = disk.read_gpt_partition_entry_array(layout, &mut entry_buf)
        .map_err(|_| GptError::IoError)?;
    ...
    let entry = entry_array.get_partition_entry_mut(slot.try_into().unwrap())
...
pub fn delete_partition<B: BlockIo>(
    block_io: B,
...
pub fn shrink_partition<B: BlockIo>(
    ...
    let entry = entry_array.get_partition_entry_mut(partition_index.try_into().unwrap())
