...
pub fn format_fat32<B: BlockIo>(
    ...
    if partition_sectors < 133120 {  // ~65MB
    ...
    block_io.write_blocks(lba, &boot_bytes).map_err(|_| Fat32Error::IoError)?;
    ...
    let fsinfo_lba = gpt_disk_types::Lba::from(gpt_disk_types::LbaLe::from_u64(partition_lba_start + 1));
    block_io.write_blocks(fsinfo_lba, &fsinfo_bytes).map_err(|_| Fat32Error::IoError)?;
    
    // Write backup boot sector (sector 6)
    let backup_lba = gpt_disk_types::Lba::from(gpt_disk_types::LbaLe::from_u64(partition_lba_start + 6));
    block_io.write_blocks(backup_lba, &boot_bytes).map_err(|_| Fat32Error::IoError)?;
    ...
    // Write first FAT
    let fat1_lba = gpt_disk_types::Lba::from(gpt_disk_types::LbaLe::from_u64(partition_lba_start + reserved_sectors as u64));
    block_io.write_blocks(fat1_lba, &fat_sector).map_err(|_| Fat32Error::IoError)?;
    
    // Write second FAT
    let fat2_lba = gpt_disk_types::Lba::from(gpt_disk_types::LbaLe::from_u64(partition_lba_start + reserved_sectors as u64 + fat_size as u64));
    block_io.write_blocks(fat2_lba, &fat_sector).map_err(|_| Fat32Error::IoError)?;
    ...
    for i in 0..sectors_per_cluster {
        let lba = gpt_disk_types::Lba::from(gpt_disk_types::LbaLe::from_u64(root_lba_val + i as u64));
        block_io.write_blocks(lba, &empty_sector).map_err(|_| Fat32Error::IoError)?;
    }
...
pub fn verify_fat32<B: BlockIo>(
    ...
    block_io.read_blocks(boot_lba, &mut buffer).map_err(|_| Fat32Error::IoError)?;
    ...
    let fsinfo_lba = gpt_disk_types::Lba::from(gpt_disk_types::LbaLe::from_u64(partition_lba_start + 1));
    block_io.read_blocks(fsinfo_lba, &mut buffer).map_err(|_| Fat32Error::IoError)?;
    ...
    let backup_lba = gpt_disk_types::Lba::from(gpt_disk_types::LbaLe::from_u64(partition_lba_start + 6));
    block_io.read_blocks(backup_lba, &mut buffer).map_err(|_| Fat32Error::IoError)?;
    ...
    let fat1_lba = gpt_disk_types::Lba::from(gpt_disk_types::LbaLe::from_u64(partition_lba_start + reserved_sectors as u64));
    block_io.read_blocks(fat1_lba, &mut buffer).map_err(|_| Fat32Error::IoError)?;
    ...
    block_io.read_blocks(fat2_lba, &mut buffer).map_err(|_| Fat32Error::IoError)?;
