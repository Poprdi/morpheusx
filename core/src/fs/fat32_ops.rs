...
impl Fat32Context {
    fn from_boot_sector<B: BlockIo>(block_io: &mut B, partition_start: u64) -> Result<Self, Fat32Error> {
        let mut boot_sector = [0u8; SECTOR_SIZE];
        block_io.read_blocks(Lba(partition_start), &mut boot_sector)
        ...
        let fat_size = u32::from_le_bytes([
            boot_sector[0x24], boot_sector[0x25], boot_sector[0x26], boot_sector[0x27]
        ]);
    ...
    fn read_fat_entry<B: BlockIo>(&self, block_io: &mut B, partition_start: u64, cluster: u32) -> Result<u32, Fat32Error> {
        ...
        block_io.read_blocks(Lba(partition_start + fat_sector as u64), &mut sector)
            .map_err(|_| Fat32Error::IoError)?;
    ...
    fn write_fat_entry<B: BlockIo>(&self, block_io: &mut B, partition_start: u64, cluster: u32, value: u32) -> Result<(), Fat32Error> {
        let fat_offset = cluster * 4;
        let fat_sector = self.reserved_sectors + (fat_offset / SECTOR_SIZE as u32);
        let entry_offset = (fat_offset % SECTOR_SIZE as u32) as usize;

        for fat_num in 0..self.num_fats {
            let sector_lba = partition_start + (self.reserved_sectors + fat_num * self.fat_size + (fat_offset / SECTOR_SIZE as u32)) as u64;
            
            let mut sector = [0u8; SECTOR_SIZE];
            block_io.read_blocks(Lba(sector_lba), &mut sector)
                .map_err(|_| Fat32Error::IoError)?;

            let masked_value = value & 0x0FFFFFFF;
            sector[entry_offset..entry_offset + 4].copy_from_slice(&masked_value.to_le_bytes());

            block_io.write_blocks(Lba(sector_lba), &sector)
                .map_err(|_| Fat32Error::IoError)?;
        }
    ...
    fn find_free_cluster<B: BlockIo>(&self, block_io: &mut B, partition_start: u64, start_from: u32) -> Result<u32, Fat32Error> {
    ...
    fn allocate_cluster<B: BlockIo>(&self, block_io: &mut B, partition_start: u64) -> Result<u32, Fat32Error> {
...
pub fn write_file<B: BlockIo>(
    ...
    for (i, part) in parts.iter().enumerate() {
        ...
        if !is_last {
            ...
            current_cluster = ensure_directory_exists(block_io, partition_lba_start, &ctx, current_cluster, part)?;
        } else {
            // This is the file name - create/write it
            write_file_in_directory(block_io, partition_lba_start, &ctx, current_cluster, part, data)?;
        }
    }
...
fn ensure_directory_exists<B: BlockIo>(
    ...
    for sec_offset in 0..ctx.sectors_per_cluster {
        ...
        block_io.read_blocks(Lba(partition_start + sector as u64 + sec_offset as u64), &mut sector_data)
            .map_err(|_| Fat32Error::IoError)?;
...
fn create_directory_in_parent<B: BlockIo>(
    ...
    let entries = unsafe {
        core::slice::from_raw_parts_mut(cluster_data.as_mut_ptr() as *mut DirEntry, 2)
    };
    entries[0] = dot_entry;
    ...
    add_dir_entry_to_cluster(block_io, partition_start, ctx, parent_cluster, name, new_cluster, 0, ATTR_DIRECTORY)?;
...
fn write_file_in_directory<B: BlockIo>(
    ...
    for i in 0..file_clusters.len() - 1 {
        ctx.write_fat_entry(block_io, partition_start, file_clusters[i], file_clusters[i + 1])?;
    }
    ...
    // Add directory entry
    add_dir_entry_to_cluster(block_io, partition_start, ctx, dir_cluster, name, file_clusters[0], data.len() as u32, ATTR_ARCHIVE)?;
...
fn add_dir_entry_to_cluster<B: BlockIo>(
    ...
    for sec_offset in 0..ctx.sectors_per_cluster {
        ...
        let entries = unsafe {
            core::slice::from_raw_parts_mut(sector_data.as_mut_ptr() as *mut DirEntry, entries_per_sector)
        };
        ...
        for entry in entries.iter_mut() {
            if entry.is_free() {
                ...
                block_io.write_blocks(Lba(partition_start + sector as u64 + sec_offset as u64), &sector_data)
                    .map_err(|_| Fat32Error::IoError)?;
...
pub fn file_exists<B: BlockIo>(
    ...
    for (i, part) in parts.iter().enumerate() {
        ...
        for sec_offset in 0..ctx.sectors_per_cluster {
            let mut sector_data = [0u8; SECTOR_SIZE];
            block_io.read_blocks(Lba(partition_lba_start + sector as u64 + sec_offset as u64), &mut sector_data)
                .map_err(|_| Fat32Error::IoError)?;
