...
impl StorageManager {
    pub(super) fn scan_disk(&mut self, disk_index: usize, bs: &BootServices) -> Result<(), usize> {
        ...
        gpt_ops::scan_partitions(adapter, &mut self.partition_table, block_size)
            .map_err(|e| {
                match e {
        ...
        if self.partition_table.has_gpt {
            let block_io = unsafe { &mut *block_io_ptr };
            let adapter = UefiBlockIoAdapter::new(block_io).map_err(|_| 1usize)?;
            self.free_space_mb = gpt_ops::calculate_total_free_space(adapter, block_size).unwrap_or(0);
        } else {
    ...
    pub(super) fn create_gpt_interactive(&mut self, screen: &mut Screen, keyboard: &mut Keyboard, bs: &BootServices) {
        screen.clear();
        screen.put_str_at(5, 5, "=== CREATE GPT PARTITION TABLE ===", EFI_LIGHTGREEN, EFI_BLACK);
        screen.put_str_at(5, 7, "WARNING: This will erase all data on the disk!", EFI_LIGHTGREEN, EFI_BLACK);
        screen.put_str_at(5, 9, "Press Y to confirm, any other key to cancel", EFI_GREEN, EFI_BLACK);
        ...
        let block_io_ptr = match crate::uefi::disk::get_disk_protocol(bs, self.current_disk_index) {
            Ok(ptr) => ptr,
            Err(_) => {
                screen.put_str_at(5, 7, "ERROR: Failed to get BlockIO protocol", EFI_LIGHTGREEN, EFI_BLACK);
        ...
        match gpt_ops::create_gpt(adapter, disk_size_lba) {
            Ok(()) => {
                ...
                screen.put_str_at(5, 7, "GPT table created successfully!", EFI_GREEN, EFI_BLACK);
                screen.put_str_at(5, 9, "Press any key to continue...", EFI_DARKGREEN, EFI_BLACK);
                keyboard.wait_for_key();
            }
            Err(e) => {
                let err_msg = match e {
                ...
                screen.put_str_at(5, 7, "ERROR: Failed to create GPT", EFI_LIGHTGREEN, EFI_BLACK);
