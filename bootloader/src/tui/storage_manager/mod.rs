...
impl StorageManager {
    ...
    fn handle_input(&mut self, key: crate::tui::input::InputKey, screen: &mut Screen, keyboard: &mut Keyboard, bs: &BootServices) -> bool {
        match self.view_mode {
            ViewMode::DiskList => {
            ...
            ViewMode::PartitionView => {
                if key.scan_code == 0x01 {
                    ...
                    let _ = self.scan_disk(self.current_disk_index, bs);
                    if self.selected_partition >= self.partition_table.count() && self.selected_partition > 0 {
                        self.selected_partition -= 1;
                    }
                    self.render(screen);
                }
        ...
        if disk_count > 0 && self.selected_disk < disk_count {
            ...
            match self.scan_disk(self.selected_disk, bs) {
                Ok(()) => {
                    if !self.partition_table.has_gpt {
                        screen.clear();
                        screen.put_str_at(5, 5, "=== NO PARTITION TABLE FOUND ===", EFI_LIGHTGREEN, EFI_BLACK);
                        screen.put_str_at(5, 7, "This disk has no GPT partition table.", EFI_GREEN, EFI_BLACK);
                        screen.put_str_at(5, 9, "Would you like to create one?", EFI_GREEN, EFI_BLACK);
                        screen.put_str_at(5, 11, "[Y] Yes, create GPT    [N] No, go back", EFI_DARKGREEN, EFI_BLACK);
                        
                        let response = keyboard.wait_for_key();
                        if response.unicode_char == b'y' as u16 || response.unicode_char == b'Y' as u16 {
                            self.create_gpt_interactive(screen, keyboard, bs);
                            let _ = self.scan_disk(self.current_disk_index, bs);
                ...
                Err(err_code) => {
                    screen.clear();
                    screen.put_str_at(5, 5, "=== ERROR SCANNING DISK ===", EFI_LIGHTGREEN, EFI_BLACK);
                    ...
                    screen.put_str_at(5, 11, "Press any key to continue...", EFI_DARKGREEN, EFI_BLACK);
                    keyboard.wait_for_key();
                    self.render(screen);
                }
            }
