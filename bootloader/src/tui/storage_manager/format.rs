...
impl StorageManager {
    pub(super) fn format_partition_ui(&mut self, screen: &mut Screen, keyboard: &mut Keyboard, bs: &BootServices) {
        ...
        if self.partition_table.count() == 0 {
            screen.put_str_at(5, 7, "ERROR: No partitions to format", EFI_LIGHTGREEN, EFI_BLACK);
        ...
        if self.selected_partition >= self.partition_table.count() {
            screen.put_str_at(5, 7, "ERROR: Invalid partition selection", EFI_LIGHTGREEN, EFI_BLACK);
            screen.put_str_at(5, 9, "Press any key...", EFI_DARKGREEN, EFI_BLACK);
            keyboard.wait_for_key();
            return;
        }
        ...
        let part = match self.partition_table.get(self.selected_partition) {
            Some(p) => p,
            None => {
                screen.put_str_at(5, 7, "ERROR: Partition not found", EFI_LIGHTGREEN, EFI_BLACK);
                screen.put_str_at(5, 9, "Press any key...", EFI_DARKGREEN, EFI_BLACK);
                keyboard.wait_for_key();
                return;
            }
        ...
        screen.put_str_at(16, 7, core::str::from_utf8(&buf[..len]).unwrap_or("?"), EFI_LIGHTGREEN, EFI_BLACK);
        ...
        screen.put_str_at(16, 9, core::str::from_utf8(&buf[..len]).unwrap_or("?"), EFI_LIGHTGREEN, EFI_BLACK);
        screen.put_str_at(16 + len, 9, " MB", EFI_LIGHTGREEN, EFI_BLACK);
        
        screen.put_str_at(5, 11, "WARNING: This will erase all data on the partition!", EFI_LIGHTGREEN, EFI_BLACK);
        ...
        screen.clear();
        screen.put_str_at(5, 5, "=== FORMATTING PARTITION ===", EFI_LIGHTGREEN, EFI_BLACK);
        ...
        let block_io_ptr = match crate::uefi::disk::get_disk_protocol(bs, self.current_disk_index) {
            Ok(ptr) => ptr,
            Err(_) => {
                screen.put_str_at(5, 9, "ERROR: Failed to access disk", EFI_LIGHTGREEN, EFI_BLACK);
        ...
        let mut adapter = match UefiBlockIoAdapter::new(block_io) {
            Ok(a) => a,
            Err(_) => {
                screen.put_str_at(5, 9, "ERROR: Failed to create adapter", EFI_LIGHTGREEN, EFI_BLACK);
                screen.put_str_at(5, 11, "Press any key...", EFI_DARKGREEN, EFI_BLACK);
                keyboard.wait_for_key();
                return;
            }
        ...
        match morpheus_core::fs::format_fat32(&mut adapter, part.start_lba, partition_sectors) {
            Ok(()) => {
                ...
                match morpheus_core::fs::verify_fat32(&mut adapter, part.start_lba) {
                    Ok(()) => {
                        screen.put_str_at(5, 11, "SUCCESS: Partition formatted as FAT32", EFI_LIGHTGREEN, EFI_BLACK);
                        screen.put_str_at(5, 12, "Filesystem integrity verified", EFI_LIGHTGREEN, EFI_BLACK);
                        screen.put_str_at(5, 14, "Press any key...", EFI_DARKGREEN, EFI_BLACK);
                    }
                    Err(_) => {
                        screen.put_str_at(5, 11, "WARNING: Format succeeded but verification failed", EFI_LIGHTGREEN, EFI_BLACK);
                        screen.put_str_at(5, 12, "Filesystem may be corrupted", EFI_LIGHTGREEN, EFI_BLACK);
                        screen.put_str_at(5, 14, "Press any key...", EFI_DARKGREEN, EFI_BLACK);
                    }
                }
            }
            Err(morpheus_core::fs::Fat32Error::PartitionTooSmall) => {
                screen.put_str_at(5, 9, "ERROR: Partition too small (min 65MB)", EFI_LIGHTGREEN, EFI_BLACK);
                screen.put_str_at(5, 11, "Press any key...", EFI_DARKGREEN, EFI_BLACK);
            }
            Err(morpheus_core::fs::Fat32Error::PartitionTooLarge) => {
                screen.put_str_at(5, 9, "ERROR: Partition too large (max 2TB)", EFI_LIGHTGREEN, EFI_BLACK);
                screen.put_str_at(5, 11, "Press any key...", EFI_DARKGREEN, EFI_BLACK);
            }
            Err(_) => {
                screen.put_str_at(5, 9, "ERROR: Failed to format partition", EFI_LIGHTGREEN, EFI_BLACK);
                screen.put_str_at(5, 11, "Press any key...", EFI_DARKGREEN, EFI_BLACK);
            }
        }
