...
use morpheus_ui::draw::shapes::{hline, rect_fill, rect_outline};
...
impl TaskManager {
    ...
    fn draw_header(&self, canvas: &mut dyn Canvas, theme: &Theme) {
        ...
        draw_string(canvas, TABLE_PAD, 3, &title, theme.title_fg, theme.title_bg, &font::FONT_DATA);
    }
    ...
    fn draw_col_labels(&self, canvas: &mut dyn Canvas, theme: &Theme) {
        ...
        draw_string(canvas, x + COL_TICKS, y, "CPU TICKS", fg, bg, &font::FONT_DATA);
    ...
    fn draw_rows(&self, canvas: &mut dyn Canvas, theme: &Theme) {
        ...
        for i in 0..self.proc_count.min(max_rows) {
            ...
            draw_string(canvas, x + COL_PID,   y, &format!("{}", p.pid), fg, bg, &font::FONT_DATA);
            ...
            draw_string(canvas, x + COL_STATE, y, state_name(p.state), sc, bg, &font::FONT_DATA);

            draw_string(canvas, x + COL_TICKS, y, &fmt_ticks(p.cpu_ticks),   fg, bg, &font::FONT_DATA);
            draw_string(canvas, x + COL_MEM,   y, &fmt_pages(p.pages_alloc), fg, bg, &font::FONT_DATA);
            draw_string(canvas, x + COL_PRIO,  y, &format!("{}", p.priority), fg, bg, &font::FONT_DATA);
        }

        if self.proc_count == 0 {
            draw_string(canvas, TABLE_PAD, t + 4, "No processes.", Color::DARK_GRAY, theme.bg, &font::FONT_DATA);
        }
    }
    ...
    fn draw_status_bar(&self, canvas: &mut dyn Canvas, theme: &Theme) {
        ...
        draw_string(canvas, TABLE_PAD, y + 3, msg, fg, theme.button_bg, &font::FONT_DATA);
    }
}
...
impl App for TaskManager {
    fn title(&self) -> &str { "Task Manager" }

    fn default_size(&self) -> (u32, u32) { (480, 340) }
