...
pub fn install_to_esp(bs: &BootServices, esp: &EspInfo, image_handle: *mut ()) -> Result<(), InstallError> {
    unsafe {
        ...
        let headers = PeHeaders::parse(image_base, image_size)
            .map_err(|_| InstallError::ProtocolError)?;
        ...
        let delta_used = headers.unrelocate_image(&mut binary_data, actual_load)
            .map_err(|_| InstallError::ProtocolError)?;
        
        // Convert from RVA layout (memory) to file layout (disk)
        let file_layout_data = headers.rva_to_file_layout(&binary_data)
            .map_err(|_| InstallError::ProtocolError)?;
        ...
        if binary_data.len() > 0x410 {
            ...
            if byte_1024 == 0xAF && byte_1025 == 0xAF && byte_1026 == 0xAF && byte_1027 == 0xAF {
                morpheus_core::logger::log("!!! Buffer already corrupted at 0x400 before FAT32 write !!!");
            } else {
...
pub fn is_installed(bs: &BootServices) -> Result<bool, InstallError> {
    ...
    unsafe {
        ...
        let exists = fat32_ops::file_exists(
            &mut adapter,
            esp.start_lba,
            "/EFI/BOOT/BOOTX64.EFI"
        ).map_err(|_| InstallError::IoError)?;
...
pub fn create_esp_and_install(
    ...
    let free_regions = morpheus_core::disk::gpt_ops::find_free_space(
        adapter,
        block_size,
    ).map_err(|_| InstallError::IoError)?;
    ...
    let part = partition_table.get(partition_index).ok_or(InstallError::IoError)?;
