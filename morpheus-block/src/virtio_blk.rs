...
// NOTE: The previous `impl From<super::virtio::init::VirtioInitError> for
// VirtioBlkInitError` was removed in Wave 4 — that source enum lived in the
// monolithic `network/` crate's NIC virtio module. The block-side path does
// its own VirtIO init inline (see `new()` / `new_pci_modern()` below).
//
// `VirtioTransport::setup_queue()` (in `morpheus-virtio`) returns
// `VirtioTransportError`; we collapse those into `VirtioBlkInitError` so the
// `?` operator works in `new_pci_modern()`.
impl From<morpheus_virtio::transport::VirtioTransportError> for VirtioBlkInitError {
    fn from(err: morpheus_virtio::transport::VirtioTransportError) -> Self {
        use morpheus_virtio::transport::VirtioTransportError as E;
        match err {
            E::QueueSetupFailed => VirtioBlkInitError::QueueSetupFailed,
        }
    }
}
